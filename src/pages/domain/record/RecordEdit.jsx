...
const RecordEdit = _ => {
    ...
    const loadRecord = async (domain, id) => {
        setLoading(true)
        let r = await api.domain.record.get(domain, id)
        setLoading(false)
        if (!r) return
        setHost(r.host)
        setType(r.type)
    ...
    const submit = async () => {
        ...
        setSaving(true)
        let r = await api.domain.record.update(domain, record)
        setSaving(false)
        if (!r) return
    ...
    React.useEffect(() => {
        appNavCtx.setBreadcrumb([
            ...
            { label: '编辑解析', link: 'domain/' + encodeURIComponent(domain) + '/record/' + encodeURIComponent(id) },
        ])
